...
ACESTEP_PATH = os.environ.get('ACESTEP_PATH')

# If not set, try to find ACE-Step path automatically
if not ACESTEP_PATH:
    # Try to find ACE-Step path by looking for acestep directory
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # Go up several levels to find the root directory
    for _ in range(5):
        if os.path.exists(os.path.join(current_dir, 'acestep')):
            ACESTEP_PATH = current_dir
            break
        current_dir = os.path.dirname(current_dir)

# If still not found, use current directory as fallback
if not ACESTEP_PATH:
    ACESTEP_PATH = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

print(f"Using ACESTEP_PATH: {ACESTEP_PATH}", file=sys.stderr)
