...
import { startDataPrep, getDataPrepTask, cancelDataPrep } from '../services/dataPrep.js';
...
// Start a one-click data preparation job (scan -> transcribe+label -> save -> preprocess).
router.post('/data-prep', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { folder, name, tag, tagPosition, hasVocals, skipMetas, onlyUnlabeled, outputDir } = req.body || {};
    if (!folder || !name) {
      return res.status(400).json({ error: 'folder and name are required' });
    }
    const task = startDataPrep({ folder, name, tag, tagPosition, hasVocals, skipMetas, onlyUnlabeled, outputDir });
    res.json(task);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
// Poll the status of a one-click data preparation job.
router.get('/data-prep-status/:id', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const task = getDataPrepTask(req.params.id);
    if (!task) {
      return res.status(404).json({ error: 'task not found' });
    }
    res.json(task);
...
// Cancel a one-click data preparation job.
router.post('/data-prep-cancel/:id', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const task = cancelDataPrep(req.params.id);
    if (!task) {
      return res.status(404).json({ error: 'task not found' });
    }
    res.json(task);
