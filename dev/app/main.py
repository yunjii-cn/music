...
def _main_log_path():
    try:
        if hasattr(sys, '_MEIPASS'):
            base = _os_for_log.path.dirname(sys.executable)
        else:
            base = _os_for_log.path.dirname(_os_for_log.path.abspath(__file__))
        return _os_for_log.path.join(base, "subprocess_debug.log")
    except Exception:
        return _os_for_log.path.join(_os_for_log.environ.get('TEMP', '.'), "subprocess_debug.log")

def _log_subprocess_call_main(method, args, kwargs):
    try:
        cmd = ""
        if args:
            cmd = str(args[0])[:200]
...
if sys.platform == 'win32':
    ...
    def _patched_popen_init(self, *args, **kwargs):
        _log_subprocess_call_main('Popen.__init__', args, kwargs)
    ...
    def _patched_run(*args, **kwargs):
        _log_subprocess_call_main('subprocess.run', args, kwargs)
        kwargs = _ensure_hidden(kwargs)
    ...
    def _patched_call(*args, **kwargs):
        _log_subprocess_call_main('subprocess.call', args, kwargs)
        kwargs = _ensure_hidden(kwargs)
    ...
    def _patched_check_call(*args, **kwargs):
        _log_subprocess_call_main('subprocess.check_call', args, kwargs)
        kwargs = _ensure_hidden(kwargs)
    ...
    def _patched_check_output(*args, **kwargs):
        _log_subprocess_call_main('subprocess.check_output', args, kwargs)
        kwargs = _ensure_hidden(kwargs)
