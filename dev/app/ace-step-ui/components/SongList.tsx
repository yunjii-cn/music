...
export const SongList: React.FC<SongListProps> = ({
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            if (isSelecting && event.key === 'Delete' && selectableSongs.length > 0 && onDeleteMany) {
                const selectedSongs = selectableSongs.filter(song => selectedIds.has(song.id));
    ...
    }, [isSelecting, selectedIds, selectableSongs, onDeleteMany]);
    ...
    const handleMouseUp = () => {
        ...
        // Calculate bounding box of drag selection
        const minX = Math.min(dragStart.x, dragEnd.x);
        const maxX = Math.max(dragStart.x, dragEnd.x);
        const minY = Math.min(dragStart.y, dragEnd.y);
        const maxY = Math.max(dragStart.y, dragEnd.y);
        
        // Select songs within the bounding box
        const newlySelected = new Set<string>();
        
        // Find all song items and check if they're within the selection box
        const songItems = document.querySelectorAll('[data-song-id]');
    ...
    return (
        ...
        >
            <div className="max-w-5xl mx-auto w-full" ref={listRef}> {/* Container constraint */}
                    ...
                    {isSelecting && (
                        <div className="flex items-center justify-between gap-3 rounded-xl border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/5 px-4 py-3">
                            <div className="text-sm text-zinc-600 dark:text-zinc-300">
                                ...
                                <span className="ml-2 text-xs text-zinc-400 dark:text-zinc-500">{t('dragToSelect')}</span>
                            </div>
                ...
                {isDragging && (
                    <div
                        className="fixed pointer-events-none z-50 border-2 border-blue-500 bg-blue-500/10 rounded-md"
                        style={{
                ...
                <div className="space-y-2"> {/* Reduced vertical spacing */}
                    {listItems.length === 0 ? (
                        ...
                        listItems.map((item) => (
                            item.type === 'song' ? (
                                <div key={item.id} data-song-id={item.song.id}>
                                    <SongItem
                                        song={item.song}
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                <div className="space-y-1">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="inline-flex items-center justify-center text-[9px] font-bold text-white bg-gradient-to-r from-blue-500 to-cyan-500 px-1.5 py-0.5 rounded-sm shadow-sm ml-1" title={`Task type: ${song.taskType || 'text2music'}`}>
                            {getTaskTypeDisplayName(song.taskType)}
                        </span>
                        {song.isPublic === false && (
