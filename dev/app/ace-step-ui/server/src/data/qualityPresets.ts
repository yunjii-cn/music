/**
 * Server-side quality presets for the "one-click training" (一键训练) feature.
 *
 * This file MIRRORS `acestep/training_v2/quick_presets.py` (the Python single
 * source of truth) AND the front-end mirror at `ace-step-ui/data/qualityPresets.ts`.
 * Keep the numeric values in sync with both.
 *
 * It is used by the Node orchestration service (`quickTrain.ts`) to compute the
 * actual training hyper-parameters sent to the Python `/v1/training/start` call.
 */
 ...
 * Mirrors `quick_presets.resolve_training_params` (Python).
 */
