...
function resolveAceStepPath(): string {
  ...
  for (let i = 0; i < 5; i++) {
    if (existsSync(path.join(currentDir, 'acestep'))) {
      console.log(`Found ACE-Step path: ${currentDir}`);
      return currentDir;
    }
  ...
  console.log(`Using CWD as ACE-Step path: ${cwd}`);
  return cwd;
}
...
async function submitToApi(params: GenerationParams): Promise<{ taskId: string }> {
  ...
  if (params.taskType === 'cover' || params.coverNoiseStrength !== undefined) {
    body.cover_noise_strength = params.coverNoiseStrength ?? 0.15;
  }
