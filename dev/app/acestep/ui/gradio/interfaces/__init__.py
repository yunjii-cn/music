...
from acestep.ui.gradio.interfaces.model_management import create_model_management_section
...
def create_gradio_interface(dit_handler, llm_handler, dataset_handler, init_params=None, language='en') -> gr.Blocks:
    ...
    with gr.Blocks(
        ...
        # Main layout: left (existing functionality) and right (model management)
        with gr.Row():
            # Left side: existing functionality (50% width)
            ...
            # Right side: model management (50% width)
            with gr.Column(scale=1):
                model_management_section = create_model_management_section(language=language)
