...
- **🚨 首跑「窗口可见但看不到」= Windows 前台锁（2026-08-01 定位）**：supervisor 经 `Popen` 拉起 entry，entry 是 **Popen 子进程、不继承前台权限**，其 `activateWindow()`→`SetForegroundWindow` 被 Windows 前台锁**静默拒绝** → `isVisible()=True` 但窗口在后台、用户只看到托盘。**第二次双击 entry（Explorer 启动）和开发模式（终端启动）自带前台权，故正常**——这正是「首跑失败、二次/开发正常」的根因。修复双保险：①supervisor `Popen` 后调 `ctypes.windll.user32.AllowSetForegroundWindow(child.pid)`；②entry 侧 `_force_foreground()` 用 `AttachThreadInput(绑前台线程输入队列)+SetForegroundWindow+BringWindowToTop+ShowWindow(SW_SHOW)` 突破前台锁，`_deferred_init` 显示后立即+500ms 双调用。**通病**：凡「Popen 拉起的 GUI 子进程」要显示前台窗口都踩此坑。
