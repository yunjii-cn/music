...
- **🚨 首跑「窗口可见但看不到」= Windows 前台锁（2026-08-01 定位）**：supervisor 经 `Popen` 拉起 entry，entry 是 **Popen 子进程、不继承前台权限**，其 `activateWindow()`→`SetForegroundWindow` 被 Windows 前台锁**静默拒绝** → `isVisible()=True` 但窗口在后台、用户只看到托盘。**第二次双击 entry（Explorer 启动）和开发模式（终端启动）自带前台权，故正常**——这正是「首跑失败、二次/开发正常」的根因。⚠️ 但 `AllowSetForegroundWindow`/`AttachThreadInput` 这套**前台锁 hack 在真机不可靠**（0722 实测仍失败：hack 在 15s 后才调用、前台权已过期）。**真正根治（0842 起）**：首跑**不 spawn 独立 entry 进程**，直接在【Explorer 拉起、自带前台权的便携 exe 进程】里落回 `_run_supervisor` 主流程跑 `main()`——显示路径与「二次双击 entry」完全一致，根除差异；便携 exe 自身由 `_run_supervisor` 末尾的 `--self-delete-helper` 子进程在退出后删除（entry 为硬链接保活数据）。**通病结论**：凡「Popen 拉起的 GUI 子进程」要显示前台窗口都踩此坑，**最稳的修法是让 UI 留在前台权进程里显示、不要 spawn 独立进程去显示**，而非依赖前台锁 hack。
