...
def build_model_device_controls(
    ...
    with gr.Row():
        # Get available models from shared directory
        available_models = dit_handler.get_available_acestep_v15_models()
        
        # Create choices with status indication
        model_choices = []
        for model in available_models:
            ...
            if os.path.exists(model_path) and os.listdir(model_path):
                # Model exists, add as normal choice
                model_choices.append(model)
            else:
                # Model doesn't exist, add as disabled choice
                model_choices.append({"label": f"{model} (不可用)", "value": model, "disabled": True})
        ...
        if "acestep-v15-turbo" in available_models:
            ...
            for model in available_models:
                ...
                if os.path.exists(model_path) and os.listdir(model_path):
                    default_model = model
                    break
            # If no available model found, use the first one
...
def build_lm_backend_controls(
    ...
    with gr.Row():
        ...
        for model in all_lm_models:
            ...
            if os.path.exists(model_path) and os.listdir(model_path):
                # Model exists, add as normal choice
                lm_model_choices.append(model)
            else:
                # Model doesn't exist, add as disabled choice
                lm_model_choices.append({"label": f"{model} (不可用)", "value": model, "disabled": True})
